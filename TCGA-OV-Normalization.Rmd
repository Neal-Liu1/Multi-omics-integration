...
data_transcriptome <- readRDS('Data/transcriptome_data.rds')
data_methylation <- readRDS('Data/methylation_data.rds')
...
colnames(transcriptome_matrix) <- substr(colnames(transcriptome_matrix),9,12)
...
colnames(proteome_matrix1) <- substr(colnames(proteome_matrix1),4,7)
...
proteome_RLE_long$Batch <- factor(rep(processed_proteome_batches2$Batch, each= nrow(proteome_RLE)))
...
colnames(transcriptome_raw_counts) <- substr(colnames(transcriptome_raw_counts), 9,12)
...
plot_RLE(methylation_RLE, processed_methylation_batch_ids$batch_id, ylimit=c(-0.2,0.2))

```

```{r}

plot_PCA(methylation_M_values,'Methylation M values',processed_methylation_batch_ids$batch_id,'batch')

```

```{r}

plot_PCA(methylation_matrix2,'methylation beta values',processed_methylation_batch_ids$batch_id,'batch')
...
`OV (Ovarian serous cystadenocarcinoma)` <- readRDS(file = 'Data/OV (Ovarian serous cystadenocarcinoma).rds')
...
transcript_batch_ftest <- ftest(as.matrix(transcriptome_matrix2),processed_transcriptome_batch_ids$batch_id,is.log=T,n.cores = 8)
methylation_batch_ftest <- ftest(as.matrix(methylation_M_values),processed_methylation_batch_ids$batch_id,is.log=T,n.cores=8)
methylation_beta_values_batch_ftest <- ftest(as.matrix(methylation_matrix2),processed_methylation_batch_ids$batch_id,is.log=T,n.cores=8)
...
methylation_vital_status_ftest <- ftest(as.matrix(methylation_M_values),patient_metadata$vital_status,is.log=T,n.cores=8)
...
transcriptome_matrix2 <- as.matrix(transcriptome_matrix2)
...
Trying to see if it works if i manually delete the batches with less than 3 samples because RUV has a bug that throws error if theres any batches with less than 3 samples.
...
patient_metadata$pseudo_batch_RLE_median_cluster <- factor(as.character(dynamic_clusters))

OV_se@colData$pseudo_batch_RLE_median_cluster <- factor(as.character(dynamic_clusters))
...
```{r}
LCM_counts <- read_delim("Data/LCM/GSE132107_counts.txt", 
    delim = "\t", escape_double = FALSE, 
    trim_ws = TRUE)

LCM_counts1 <- as.data.frame(LCM_counts[,-1])
rownames(LCM_counts1) <- LCM_counts$Genes
LCM_counts2 <- LCM_counts1
```

```{r}
LCM_counts2 <- remove_all_zero_variance_rows(LCM_counts2)

a <- filterByExpr(y=LCM_counts1, min.count = 4, min.total.count = 10)
...
transcript_fpkm_uq_vital_stats_wilcox_test <- wilcoxon_test(matrix=transcriptome_fpkm_uq1, variable=data_transcriptome1@colData$vital_status, is.log=F)
...
Now, since we have a significant number of missing tumour purity values, and we also need to estimate purity for LCM data anyway,
we're gonna take matters into our own hands and figure out how to calculate purity ourselves.

```{r}

Yoshihara_stromal_immune_signatures <- read_excel("Data/Yoshihara et al stromal immune signatures.xlsx")

stromal_immune_signatures <- as.data.frame(t(Yoshihara_stromal_immune_signatures[,-(1:2)]))
rownames(stromal_immune_signatures) <- stromal_immune_signatures$V1
stromal_immune_signatures <- stromal_immune_signatures[,-1]
colnames(stromal_immune_signatures) <- c('Stromal','Immune')

stromal_immune_signatures[stromal_immune_signatures =='TXNDC3'] <- 'NME8'
stromal_immune_signatures[stromal_immune_signatures =='ODZ4'] <- 'TENM4'

ensembl <- useMart("ensembl", host="https://asia.ensembl.org", dataset = "hsapiens_gene_ensembl")

stromal_immune_genes <- getBM(attributes = c('hgnc_symbol', 'ensembl_gene_id','entrezgene_id'),
                      filters = 'hgnc_symbol',
                      values = cbind(stromal_immune_signatures$Stromal, stromal_immune_signatures$Immune),
                      mart = ensembl)


transcriptome_fpkm_uq2 <- transcriptome_fpkm_uq1

colnames(transcriptome_fpkm_uq2) <- substr(colnames(transcriptome_fpkm_uq1),9,12)
rownames(transcriptome_fpkm_uq2) <- substr(rownames(transcriptome_fpkm_uq2),1,15)

# 6 patients are duplicated so we're removing them here.
transcriptome_fpkm_uq2 <- transcriptome_fpkm_uq2[,!duplicated(colnames(transcriptome_fpkm_uq2))]

rank_data_temp <- singscore::rankGenes(transcriptome_fpkm_uq2)

purity_test <- singscore::simpleScore(rank_data_temp, 
                                      upSet = as.vector(unique(stromal_immune_genes$ensembl_gene_id)),
                                      centerScore = F,
                                      knownDirection = T)

calculated_singscore_purity <- data.frame(`edgeR_filterbyExpr(10,25)` = (1-purity_test$TotalScore[match(data_transcriptome1@colData@rownames, rownames(purity_test))]),
                                          original_purity_HTseq_FPKM_UQ = data_transcriptome1@colData$purity_HTseq_FPKM_UQ)
```

```{r}

# transcriptome_fpkm_uq3 <- transcriptome_fpkm_uq[filterByExpr(assay(data_transcriptome,'unstranded'), min.count = 30, min.total.count = 100),]


keep.genes.cancer <- apply(assay(data_transcriptome,'unstranded'), 1, function(x) length(x[x > 15]) >= ncol(transcriptome_fpkm_uq)*.1)

transcriptome_fpkm_uq3 <- transcriptome_fpkm_uq[keep.genes.cancer,]
colnames(transcriptome_fpkm_uq3) <- substr(colnames(transcriptome_fpkm_uq3),9,12)
rownames(transcriptome_fpkm_uq3) <- substr(rownames(transcriptome_fpkm_uq3),1,15)
transcriptome_fpkm_uq3 <- transcriptome_fpkm_uq3[,!duplicated(colnames(transcriptome_fpkm_uq3))]

purity_test1 <- singscore::simpleScore(rankGenes(transcriptome_fpkm_uq3), 
                                      upSet = as.vector(unique(stromal_immune_genes$ensembl_gene_id)),
                                      centerScore = F,
                                      knownDirection = T)

calculated_singscore_purity$`number of count > 15 greater than 10% ncol` <- (1-purity_test1$TotalScore[match(data_transcriptome1@colData@rownames, rownames(purity_test1))])
...
a<- reshape2::melt(calculated_singscore_purity)


# Plotting density for each variable
ggplot(a, aes(x = value, fill = variable, color=variable)) +
  geom_density(alpha = 0.65) +
  theme_minimal() + 
  labs(x='Purity', title = 'Distribution of purity estimates with different gene filtering methods')+
  theme(axis.line = element_line(colour = "grey75",linewidth = 0.8, lineend='round'),
        panel.grid.major = element_line(color = "grey96")
        )
...
# Plot the density plots
ggplot(mapping = aes(x=calculated_singscore_purity$edgeR_filterbyExpr.10.25., fill=data_transcriptome1@colData$subtype)) +
  geom_density(alpha=0.5) + # alpha for transparency to see overlap
  labs(x="Tumor Purity", y="Density", fill="Tumor Subtype") +
  ggtitle('Distribution of our calculated fpkm_uq purity estimate for each subtype') +
  theme_minimal() +
  scale_fill_brewer(palette="Set1")+
  xlim(0.25, 0.75)+
  theme(panel.border=element_rect(colour = "grey80", fill=NA, size=0.8))
...
```{r}
ggplot(mapping = aes(x=log(transcriptome_fpkm_uq1+1)['ENSG00000043462.13',], y= calculated_singscore_purity$edgeR_filterbyExpr.10.25., color= data_transcriptome1@colData$subtype)) +
  geom_point(alpha=0.7) + # alpha for transparency to see overlap
  labs(x="Expression levels of ENSG00000043462 (log fpkm uq)", y="Calculated singscore purity", color="Subtypes") +
  ggtitle('Expression levels of Lymphocyte cytosolic protein 2 vs purity') +
  theme_minimal() +
  scale_fill_brewer(palette="Set1")+
  theme(panel.border=element_rect(colour = "grey80", fill=NA, size=0.8),
...
ggplot(mapping = aes(x=log(transcriptome_fpkm_uq1+1)['ENSG00000167771.6',], y= calculated_singscore_purity$edgeR_filterbyExpr.10.25., color= data_transcriptome1@colData$subtype)) +
  geom_point(alpha=0.72) + # alpha for transparency to see overlap
  labs(x="Expression levels of ENSG00000167771.6 (log fpkm uq)", y="HTseq purity", color="Subtypes") +
  ggtitle('Expression levels of REST corepressor 2 (RCOR2) vs purity') +
  theme_minimal() +
...
ggplot(mapping = aes(x = calculated_singscore_purity$edgeR_filterbyExpr.10.25., y = OV_subtypes1$`1st&2nd_subtype_difference`, color=OV_subtypes1$Subtype)) +
   geom_point(alpha=0.7) +
   labs(x = 'Calculated singscore Purity', y = 'difference between 1st & 2nd most likely subtypes') +
   ggtitle('Calculated singscore Purity vs certainty of subtype classifier (TCGA log fpkm)')+
   theme_minimal() +
   theme(panel.border=element_rect(colour = "grey80", fill=NA, size=0.8),
        aspect.ratio = 1/1.4,
        axis.line = element_line(colour = "grey75", linewidth = 1.1),
...
Convert LCM data into fpkm_uq and then try to do purity estimation to see how pure the LCM data is

```{r}
gene_lengths <- getBM(attributes = c('hgnc_symbol', 'gene_biotype', 'chromosome_name', 'start_position', 'end_position','ensembl_gene_id','entrezgene_id'),
                      filters = 'hgnc_symbol',
                      values = rownames(LCM_counts1),
                      mart = ensembl)
...
gene_lengths <- gene_lengths[!duplicated(gene_lengths$hgnc_symbol),]

rownames(gene_lengths) <- gene_lengths[,1]

LCM_counts3 <- lineup_samples(list(LCM_counts1[edgeR::filterByExpr(LCM_counts1,5,20),], gene_lengths), direction = 'rows')[[1]]
gene_lengths <- lineup_samples(list(LCM_counts1[edgeR::filterByExpr(LCM_counts1,5,20),], gene_lengths), direction = 'rows')[[2]]
...
# convert the raw counts to fpkm_uq

gene_lengths$gene_lengths_kb <- gene_lengths$length / 1000
normalized_counts <- sweep(LCM_counts3, 1, gene_lengths$gene_lengths_kb, FUN = "/")

upper_quartile <- as.vector(apply(normalized_counts, 1, function(x) quantile(x, probs=0.75, na.rm=TRUE))+1e-9)

LCM_fpkm_uq <- sweep(normalized_counts, 1, upper_quartile, FUN = "/") * 1e6
...
rank_data_temp <- singscore::rankGenes(LCM_counts1[edgeR::filterByExpr(LCM_counts1,min.count = 4, min.total.count = 10),])

purity_test1 <- singscore::simpleScore(rank_data_temp, 
                                      upSet = as.vector(unique(stromal_immune_genes$hgnc_symbol)),
                                      centerScore = F,
                                      knownDirection = T)

purity_test1$Purity <- (1-purity_test1$TotalScore)

ggplot(mapping = aes(x=purity_test1$Purity))+
  geom_density(fill='grey80', color='grey70', alpha=0.3)+
  theme_minimal() +
  theme(axis.line = element_line(colour = "grey83", linewidth = 1.1),
          panel.border = element_rect(colour = "grey90", fill=NA, size=0.7),
          panel.grid.major = element_line(color = "grey96"),
          aspect.ratio = 1/1.4) +
    ggtitle('Distribution of Adj.Pvalues for subtype & purity two-way ANOVA')
```


```{r}

purity_test1 <- singscore::simpleScore(singscore::rankGenes(LCM_fpkm_uq), 
                                      upSet = as.vector(unique(stromal_immune_genes$hgnc_symbol)),
                                      centerScore = F,
                                      knownDirection = T)
...
genes <- substr(rownames(assay(data_transcriptome1,'transcriptome_matrix1')),1,15)

gene_orders <- match(genes, entrez_ids$ensembl_gene_id)

entrez_ids2 <- entrez_ids[gene_orders,]

OV_subtypes <- consensusOV::get.subtypes(as.matrix(log(assay(data_transcriptome1,'transcriptome_matrix1')+1)),entrez.ids = entrez_ids2$entrezgene_id)
...
OV_subtypes1$Purity <- data_transcriptome2@colData$purity_HTseq_FPKM_UQ

data_transcriptome1@colData$Calculated_singscore_purity <- calculated_singscore_purity$edgeR_filterbyExpr.10.25.
```
...
ggplot(mapping = aes(x=data_transcriptome1@colData$purity_HTseq_FPKM_UQ, y=data_transcriptome1@colData$Calculated_singscore_purity))+
  geom_point( alpha = 0.4, color = 'grey30',)+
  theme_minimal() +
  labs(x = 'Original purity', y = 'Calculated purity') +
  theme(axis.line = element_line(colour = "grey83", linewidth = 1.1),
          panel.border = element_rect(colour = "grey90", fill=NA, size=0.7),
          panel.grid.major = element_line(color = "grey96"),
          aspect.ratio = 1/1.23) +
    ggtitle('Original fpkm_uq purity vs our calculated purity')
...
OV_subtypes1$Confident_subtypes <- ifelse(OV_subtypes1$`1st&2nd_subtype_difference` > 0.4, 'yes', 'no')

data_transcriptome1@colData$Confident_subtypes <- OV_subtypes1$Confident_subtypes

plot_PCA(assay(data_transcriptome1,'transcriptome_matrix1')[rownames(transcript_subtype_kruskal_test)[rownames(transcript_subtype_kruskal_test) %in% rownames(assay(data_transcriptome1, 'transcriptome_matrix1'))][1:400],],
         'top 400 genes highly differeing between subtypes', 
         data_transcriptome1@colData$Confident_subtypes,
         'confident subtypes', 
         pcs=c(1,2))
...
data_transcriptome1@colData$Binned_purity <- cut(data_transcriptome1@colData$Calculated_singscore_purity,
         breaks=quantile(data_transcriptome1@colData$Calculated_singscore_purity, probs=0:4/4),
         include.lowest=TRUE, labels=F)
 
transcript_subtype_purity_anova <- ANOVA_2way(assay(data_transcriptome1,'transcriptome_matrix1'), data_transcriptome1@colData$subtype, data_transcriptome1@colData$Binned_purity)

ggplot(mapping=aes(x=transcript_subtype_purity_anova$Interaction_Adj_PValue))+
  geom_histogram(fill='grey70', color='grey60', alpha=0.5)+
  theme_minimal() +
  theme(axis.line = element_line(colour = "grey83", linewidth = 1.1),
          panel.border = element_rect(colour = "grey90", fill=NA, size=0.7),
          panel.grid.major = element_line(color = "grey96"),
...
plot_PCA(assay(data_transcriptome1,'transcriptome_matrix1')[which(transcript_subtype_purity_anova$Interaction_Adj_PValue < 0.3),],'genes with high interaction between purity & subtype',data_transcriptome1@colData$subtype,'subtype', pcs=c(1,2))
...
plot_ly(data = as.data.frame(prcomp(assay(data_transcriptome1,'transcriptome_matrix1')[which(transcript_subtype_purity_anova$Interaction_Adj_PValue < 0.3),])[[2]]), 
        x = ~PC1, y = ~PC2, z = ~PC3, color = data_transcriptome1@colData$subtype, 
...
ggplot(mapping=aes(x=transcript_subtype_purity_anova$Interaction_Adj_PValue[which(transcript_subtype_purity_anova$Var1_Adj_PValue < 0.01 & transcript_subtype_purity_anova$Var2_Adj_PValue > 0.9)]))+
  geom_histogram(fill='grey70', color='grey60', alpha=0.5)+
  theme_minimal() +
  theme(axis.line = element_line(colour = "grey83", linewidth = 1.1),
          panel.border = element_rect(colour = "grey90", fill=NA, size=0.7),
...
```{r}
plot_PCA(assay(data_transcriptome1,'transcriptome_matrix1')[which(transcript_subtype_purity_anova$Var1_Adj_PValue < 0.05 & transcript_subtype_purity_anova$Var2_Adj_PValue > 0.95 & transcript_subtype_purity_anova$Interaction_Adj_PValue > 0.95),],'genes that differ between subtype & purity',data_transcriptome1@colData$subtype,'subtype', pcs=c(1,2))

```

```{r}

ggplot(mapping=aes(x=transcript_subtype_purity_anova$Interaction_Adj_PValue[which(transcript_subtype_purity_anova$Var1_Adj_PValue < 0.01 & transcript_subtype_purity_anova$Var2_Adj_PValue < 0.01)]))+
  geom_histogram(fill='grey70', color='grey60', alpha=0.5)+
  theme_minimal() +
  theme(axis.line = element_line(colour = "grey80", linewidth = 1.1),
          panel.border = element_rect(colour = "grey90", fill=NA, size=0.7),
          panel.grid.major = element_line(color = "grey96"),
          aspect.ratio = 1/1.4) +
    ggtitle('Distribution of Adj.Pvalues for subtype & purity two-way ANOVA')
...
```{r}
genes_w_subtype_purity_interaction_info <- getBM(attributes = c('ensembl_gene_id', 'entrezgene_id','external_gene_name', 'gene_biotype',
                                                                'description'   # 'name_1006', 'go_linkage_type'
                                                                ),
                   filters = 'ensembl_gene_id', 
                   values = substr(rownames(transcript_subtype_purity_anova[which(transcript_subtype_purity_anova$Interaction_Adj_PValue<0.05),]),1,15), 
                   mart = ensembl)

rownames(genes_w_subtype_purity_interaction_info) <- rownames(transcript_subtype_purity_anova[which(transcript_subtype_purity_anova$Interaction_Adj_PValue<0.05),])
...
ggplot(mapping = aes(x=log(transcriptome_fpkm_uq[,colnames(transcriptome_fpkm_uq) %in% colnames(transcriptome_fpkm_uq1)]+1)["ENSG00000011600.12",], 
                     y= calculated_singscore_purity$edgeR_filterbyExpr.10.25., color= data_transcriptome1@colData$subtype)) +
  geom_point(alpha=0.72) + # alpha for transparency to see overlap
  labs(x="Expression levels of ENSG00000128564.7 (log fpkm uq)", 
       y="HTseq purity", 
       color="Subtypes") +
  ggtitle('Expression levels of VGF nerve growth factor vs purity') +
  theme_minimal() +
  scale_fill_brewer(palette="Set1")+
  theme(panel.border=element_rect(colour = "grey80", fill=NA, size=0.8),
        aspect.ratio = 1/1.05,
...
plot_boxplot_categorical(log(transcriptome_fpkm_uq[,colnames(transcriptome_fpkm_uq) %in% colnames(transcriptome_fpkm_uq1)]+1)["ENSG00000011600.12",],
                         data_transcriptome1@colData$subtype,
                         c('Transmembrane immune signaling adaptor TYROBP', 'subtype'), aspect_ratio = 1.2) +
  labs(y= 'Expression (log fpkm_uq)')
...
ego <- enrichGO(gene = genes_w_subtype_purity_interaction_info$ensembl_gene_id,
                OrgDb = 'org.Hs.eg.db', 
                keyType = "ENSEMBL", 
                ont = "BP", 
                pAdjustMethod = "BH",
...
enrichplot::dotplot(ego, label_format = 100) +
  theme(aspect.ratio = 1/0.5)
  

```
```{r,fig.width= 11}
genes_w_subtype_purity_interaction_info1 <- getBM(attributes = c('ensembl_gene_id', 'entrezgene_id','external_gene_name', 'gene_biotype',
                                                                'description'   # 'name_1006', 'go_linkage_type'
                                                                ),
                   filters = 'ensembl_gene_id', 
                   values = substr(rownames(transcript_subtype_purity_anova[which(transcript_subtype_purity_anova$Interaction_Adj_PValue<0.3),]),1,15), 
                   mart = ensembl)


enrichplot::dotplot(enrichGO(gene = genes_w_subtype_purity_interaction_info1$ensembl_gene_id,
                OrgDb = 'org.Hs.eg.db', 
                keyType = "ENSEMBL", 
                ont = "BP", 
                pAdjustMethod = "BH",
...
Try to use RUV-III on only the very confidently subtyped samples. Since the package does not allows us to specifically exclude samples in PRPS, we have to do it manually. 

```{r}

data_transcriptome1@colData$Confident_subtypes <- ifelse(OV_subtypes1$`1st&2nd_subtype_difference` > 0.4, 'yes', 'no')
data_transcriptome1@colData$log2_libsize <- log2(data_transcriptome1@colData$lib_size)

PRPS_confident_subtypes <- supervisedPRPS(se.obj=data_transcriptome1[,which(data_transcriptome1@colData$Confident_subtypes == 'yes')],
                       assay.name='transcriptome_matrix1',
                       bio.variable= 'subtype',
                       uv.variables=c('log2_libsize','batch','Calculated_singscore_purity'),
...
Plotting PRPS map to see if there's sufficient PRPS

```{r}

prps_map <- as.data.frame(data_transcriptome1[,which(data_transcriptome1@colData$Confident_subtypes == 'yes')]@colData)[,c('batch','subtype')] %>%
...
prps_map_original <- as.data.frame(data_transcriptome1@colData)[,c('batch','subtype')] %>%
    dplyr::count(batch, subtype)

prps_map_original$use <- ifelse(prps_map_original$n > 2, 'yes','no')

ggplot(prps_map_original, aes(x = batch, y = subtype)) +
  geom_count(aes(color = use)) +
  geom_text(aes(
    label = n,
    hjust = 0.5,
...
PRPS_metadata <- PRPS_confident_subtypes@metadata
PRPS_confident_subtypes <- data_transcriptome1
PRPS_confident_subtypes@metadata <- PRPS_metadata


PRPS_confident_subtypes <- supervisedFindNCG(se.obj=PRPS_confident_subtypes,
                         assay.name= 'transcriptome_matrix1',
                         bio.variables= 'subtype',
                         uv.variables= c('log2_libsize','batch','Calculated_singscore_purity'),
                         no.ncg = 1000,
                         regress.out.uv.variables = F,
                         regress.out.bio.variables = F,
                         apply.normalization= F,
                         normalization =  'CPM',
                         assess.se.obj = T,
                         apply.log = F,
                         pseudo.count = 1,
...
replicate.data=t(do.call(cbind, PRPS_confident_subtypes@metadata[['PRPS']][['supervised']]))

PRPS_confident_subtypes <- ruvIIIMultipleK(se.obj=PRPS_confident_subtypes,
                    assay.name='transcriptome_matrix1',
                    apply.log=F,
                    pseudo.count = 1,
                    replicate.data=replicate.data,
                    ctl=PRPS_confident_subtypes@metadata[['NCG']],
                    k = 4,
                    eta = NULL,
                    include.intercept = T,
                    apply.average.rep = F,
...
p1 <- plot_vector_correlation(list(prcomp(t(assay(PRPS_confident_subtypes,'transcriptome_matrix1'))),
                             prcomp(t(assay(PRPS_confident_subtypes,'RUV_K4_on_transcriptome_matrix1')))),
                        c('unnormalized','RUV-K4'),
                        PRPS_confident_subtypes@colData$subtype,
                        'subtypes'
                        )

p2 <- plot_vector_correlation(list(prcomp(t(assay(PRPS_confident_subtypes,'transcriptome_matrix1'))),
                             prcomp(t(assay(PRPS_confident_subtypes,'RUV_K4_on_transcriptome_matrix1')))),
                        c('unnormalized','RUV-K4'),
                        PRPS_confident_subtypes@colData$batch,
                        'batch'
                        )

p3 <- plot_linear_correlation(PRPS_confident_subtypes@assays@data@listData,
                        c('unnormalized','RUV-K4'),
                        PRPS_confident_subtypes@colData$log2_libsize,
                        'log2 libsize')


p4 <- plot_linear_correlation(PRPS_confident_subtypes@assays@data@listData,
                        c('unnormalized','RUV-K4'),
                        PRPS_confident_subtypes@colData$Calculated_singscore_purity,
                        'purity')
...
NCGs <- rownames(assay(PRPS_confident_subtypes,'transcriptome_matrix1'))[PRPS_confident_subtypes@metadata$NCG]

NCG_pca <- as.data.frame(list(BiocSingular::runSVD(t(assay(PRPS_confident_subtypes,'transcriptome_matrix1')[NCGs,]),k=3,center=T))[[1]]$u)

```

```{r, fig.width= 8}

p1 <- ggplot(mapping = aes(x=log2(PRPS_confident_subtypes@colData$lib_size),y=NCG_pca$V1))+
  geom_point(alpha=0.62, color= 'grey30') + # alpha for transparency to see overlap
  labs(y="PC1 of NCGs", 
       x="Log2 lib-size") +
  theme_minimal()+
  ggtitle('1st PC of negative control genes vs lib size') +
  theme(panel.border=element_rect(colour = "grey80", fill=NA, size=0.8),
        aspect.ratio = 1/1.05,
        axis.line = element_line(colour = "grey75", linewidth = 1.1),
        panel.grid.major = element_line(color = "grey96"),)

p2 <- ggplot(mapping = aes(x= PRPS_confident_subtypes@colData$Calculated_singscore_purity,y=NCG_pca$V1))+
  geom_point(alpha=0.62, color= 'grey30') + # alpha for transparency to see overlap
  labs(y="PC1 of NCGs", 
       x="Purity") +
  theme_minimal()+
...
raw_counts <- assay(data_transcriptome,'unstranded')[,!data_transcriptome@colData$batch %in% batches_to_remove]
colnames(raw_counts) <- substr(colnames(raw_counts),9,12)
a <- edgeR::filterByExpr(assay(data_transcriptome,'fpkm_unstrand'),10,30)
raw_counts <- raw_counts[a,]

PRPS_confident_subtypes_raw_counts <- SummarizedExperiment(assays= list(raw_counts = raw_counts), colData = data_transcriptome1@colData)

PRPS_confident_subtypes_raw_counts@colData$log2_libsize <- log2(PRPS_confident_subtypes_raw_counts@colData$lib_size)

PRPS_confident_subtypes_raw_counts1 <- PRPS_confident_subtypes_raw_counts

PRPS_confident_subtypes_raw_counts <- supervisedPRPS(se.obj=PRPS_confident_subtypes_raw_counts[,which(PRPS_confident_subtypes_raw_counts@colData$Confident_subtypes == 'yes')],
                       assay.name='raw_counts',
                       bio.variable= 'subtype',
                       uv.variables=c('log2_libsize','batch','Calculated_singscore_purity'),
                       batch.variable = 'batch',
                       min.sample.for.prps = 3,
...
PRPS_metadata <- PRPS_confident_subtypes_raw_counts@metadata
PRPS_confident_subtypes_raw_counts <- PRPS_confident_subtypes_raw_counts1
PRPS_confident_subtypes_raw_counts@metadata <- PRPS_metadata


PRPS_confident_subtypes_raw_counts <- supervisedFindNCG(se.obj=PRPS_confident_subtypes_raw_counts,
                         assay.name= 'raw_counts',
                         bio.variables= 'subtype',
                         uv.variables= c('log2_libsize','batch','Calculated_singscore_purity'),
                         no.ncg = 1000,
...
replicate.data=t(do.call(cbind, PRPS_confident_subtypes_raw_counts@metadata[['PRPS']][['supervised']]))


PRPS_confident_subtypes_raw_counts <- ruvIIIMultipleK(se.obj=PRPS_confident_subtypes_raw_counts,
                    assay.name='raw_counts',
                    apply.log=T,
                    pseudo.count = 1,
                    replicate.data=replicate.data,
                    ctl=PRPS_confident_subtypes_raw_counts@metadata[['NCG']],
                    k = 4,
                    eta = NULL,
                    include.intercept = T,
                    apply.average.rep = F,
...
NCGs <- rownames(assay(PRPS_confident_subtypes_raw_counts,'raw_counts'))[PRPS_confident_subtypes_raw_counts@metadata$NCG]

NCG_pca_raw_counts <- as.data.frame(list(BiocSingular::runSVD(t(raw_counts[NCGs,]),k=3,center=T))[[1]]$u)

p1 <- ggplot(mapping = aes(x=log2(PRPS_confident_subtypes_raw_counts@colData$log2_libsize),y=NCG_pca_raw_counts$V1))+
  geom_point(alpha=0.62, color= 'grey30') + # alpha for transparency to see overlap
  labs(y="PC1 of NCGs", 
       x="Log2 lib-size") +
  theme_minimal()+
...
p2 <- ggplot(mapping = aes(x=PRPS_confident_subtypes_raw_counts@colData$Calculated_singscore_purity,y=NCG_pca_raw_counts$V1))+
  geom_point(alpha=0.62, color= 'grey30') + # alpha for transparency to see overlap
  labs(y="PC1 of NCGs", 
       x="Purity") +
  theme_minimal()+
...
p1 <- plot_vector_correlation(list(prcomp(t(assay(PRPS_confident_subtypes_raw_counts,'raw_counts'))),
                             prcomp(t(assay(PRPS_confident_subtypes_raw_counts,'RUV_K4_on_raw_counts')))),
                        c('unnormalized','RUV-K5'),
                        PRPS_confident_subtypes_raw_counts@colData$subtype,
                        'subtypes'
                        )

p2 <- plot_vector_correlation(list(prcomp(t(assay(PRPS_confident_subtypes_raw_counts,'raw_counts'))),
                             prcomp(t(assay(PRPS_confident_subtypes_raw_counts,'RUV_K4_on_raw_counts')))),
                        c('unnormalized','RUV-K5'),
                        PRPS_confident_subtypes_raw_counts@colData$batch,
                        'batch'
                        )

p3 <- plot_linear_correlation(PRPS_confident_subtypes_raw_counts@assays@data@listData,
                        c('unnormalized','RUV-K5'),
                        PRPS_confident_subtypes_raw_counts@colData$log2_libsize,
                        'log2 libsize')

p4 <- plot_linear_correlation(PRPS_confident_subtypes_raw_counts@assays@data@listData,
                        c('unnormalized','RUV-K5'),
                        PRPS_confident_subtypes_raw_counts@colData$Calculated_singscore_purity,
                        'purity')
...
plot_PCA(assay(PRPS_confident_subtypes_raw_counts,'RUV_K4_on_raw_counts'),'RUV-K4 raw counts',PRPS_confident_subtypes_raw_counts@colData$subtype,'subtypes', pcs= c(1,2))
...
plot_ly(data = as.data.frame(prcomp(assay(PRPS_confident_subtypes_raw_counts,'RUV_K4_on_raw_counts'))[[2]]), 
        x = ~PC1, y = ~PC2, z = ~PC3, color = data_transcriptome1@colData$subtype, 
        type = 'scatter3d', mode = 'markers',
        marker = list(size = 4, opacity = 0.8)) %>%
  layout(title = 'PCA of RUV-K4 raw counts',
         scene = list(xaxis = list(title = 'PC1'),
                      yaxis = list(title = 'PC2'),
                      zaxis = list(title = 'PC3')))
```
...
plot_RLE(assay(PRPS_confident_subtypes_raw_counts,'RUV_K4_on_raw_counts')[,!duplicated(colnames(assay(PRPS_confident_subtypes_raw_counts,'raw_counts')))], PRPS_confident_subtypes_raw_counts@colData$batch[!duplicated(colnames(assay(PRPS_confident_subtypes_raw_counts,'raw_counts')))], ylimit = c(-4,4))

```

```{r, fig.height=6,fig.width=19}

plot_RLE(log2(assay(PRPS_confident_subtypes_raw_counts,'raw_counts')+1)[,!duplicated(colnames(assay(PRPS_confident_subtypes_raw_counts,'raw_counts')))], PRPS_confident_subtypes_raw_counts@colData$batch[!duplicated(colnames(assay(PRPS_confident_subtypes_raw_counts,'raw_counts')))], ylimit = c(-4,4))

```

Normalizing the methylation

```{r, fig.height=6,fig.width=19}

methylation_M_values <- log2(methylation_matrix1/(1-methylation_matrix1))
methylation_M_values <- methylation_M_values[,!duplicated(colnames(methylation_M_values))]
methylation_batches1 <- as.data.frame(methylation_batches[!duplicated(substr(methylation_batches$aliquot_barcode,9,12)),])

rownames(methylation_batches1) <- substr(methylation_batches1$aliquot_barcode,9,12)

methylation_M_values1 <- lineup_samples(list(methylation_M_values, t(methylation_batches1), PRPS_confident_subtypes_raw_counts))[[1]]
methylation_batches1 <- lineup_samples(list(methylation_M_values, t(methylation_batches1), PRPS_confident_subtypes_raw_counts))[[2]]

PRPS_confident_subtypes_aligned <- lineup_samples(list(methylation_M_values, methylation_batches1, PRPS_confident_subtypes_raw_counts))[[3]]
methylation_batches2 <- as.data.frame(t(methylation_batches1))

plot_RLE(methylation_M_values1, methylation_batches2$batch_id, ylim=c(-4, 4))
...
PRPS_confident_subtypes_M_values <- SummarizedExperiment(assays = list(M_values = as.matrix(methylation_M_values1)), colData = PRPS_confident_subtypes_aligned@colData)

PRPS_confident_subtypes_M_values@colData$batch <- methylation_batches2$batch_id

PRPS_confident_subtypes_M_values1 <- PRPS_confident_subtypes_M_values

PRPS_confident_subtypes_M_values <- supervisedPRPS(se.obj=PRPS_confident_subtypes_M_values[,which(PRPS_confident_subtypes_M_values@colData$Confident_subtypes == 'yes')],
                       assay.name='M_values',
                       bio.variable= 'subtype',
                       uv.variables=c('batch','Calculated_singscore_purity'),
                       batch.variable = 'batch',
                       min.sample.for.prps = 3,
                       min.sample.per.batch= 3,
                       apply.log = F,
...
PRPS_metadata <- PRPS_confident_subtypes_M_values@metadata
PRPS_confident_subtypes_M_values <- PRPS_confident_subtypes_M_values1
PRPS_confident_subtypes_M_values@metadata <- PRPS_metadata


PRPS_confident_subtypes_M_values <- supervisedFindNCG(se.obj=PRPS_confident_subtypes_M_values,
                         assay.name= 'M_values',
                         bio.variables= 'subtype',
                         uv.variables= c('batch','Calculated_singscore_purity'),
                         no.ncg = 1000,
...
replicate.data=t(do.call(cbind, PRPS_confident_subtypes_M_values@metadata[['PRPS']][['supervised']]))


PRPS_confident_subtypes_M_values <- ruvIIIMultipleK(se.obj=PRPS_confident_subtypes_M_values,
                    assay.name='M_values',
                    apply.log=F,
                    pseudo.count = 1,
                    replicate.data=replicate.data,
                    ctl=PRPS_confident_subtypes_M_values@metadata[['NCG']],
                    k = 12,
                    eta = NULL,
                    include.intercept = T,
                    apply.average.rep = F,
...
NCGs <- rownames(assay(PRPS_confident_subtypes_M_values,'M_values'))[PRPS_confident_subtypes_M_values@metadata$NCG]

NCG_pca_M_values <- as.data.frame(list(BiocSingular::runSVD(t(assay(PRPS_confident_subtypes_M_values,'M_values')[NCGs,]),k=3,center=T))[[1]]$u)

p1 <- ggplot(mapping = aes(x=log2(PRPS_confident_subtypes_M_values@colData$log2_libsize),y=NCG_pca_M_values$V1))+
  geom_point(alpha=0.62, color= 'grey30') + # alpha for transparency to see overlap
  labs(y="PC1 of NCGs", 
       x="Log2 lib-size") +
  theme_minimal()+
...
p2 <- ggplot(mapping = aes(x=log2(PRPS_confident_subtypes_M_values@colData$Calculated_singscore_purity),y=NCG_pca_M_values$V1))+
  geom_point(alpha=0.62, color= 'grey30') + # alpha for transparency to see overlap
  labs(y="PC1 of NCGs", 
       x="Purity") +
  theme_minimal()+
...
p1 <- plot_vector_correlation(list(prcomp(t(assay(PRPS_confident_subtypes_M_values,'M_values'))),
                             prcomp(t(assay(PRPS_confident_subtypes_M_values,'RUV_K12_on_M_values')))),
                        c('unnormalized','RUV-K12'),
                        PRPS_confident_subtypes_M_values@colData$subtype,
                        'subtypes'
                        )

p2 <- plot_vector_correlation(list(prcomp(t(assay(PRPS_confident_subtypes_M_values,'M_values'))),
                             prcomp(t(assay(PRPS_confident_subtypes_M_values,'RUV_K12_on_M_values')))),
                        c('unnormalized','RUV-K12'),
                        PRPS_confident_subtypes_M_values@colData$batch,
                        'batch'
                        )

p3 <- plot_linear_correlation(PRPS_confident_subtypes_M_values@assays@data@listData,
                        c('unnormalized','RUV-K12'),
                        PRPS_confident_subtypes_M_values@colData$log2_libsize,
                        'log2 libsize')

p4 <- plot_linear_correlation(PRPS_confident_subtypes_M_values@assays@data@listData,
                        c('unnormalized','RUV-K12'),
                        PRPS_confident_subtypes_M_values@colData$Calculated_singscore_purity,
                        'purity')
...
plot_3D_PCA(assay(PRPS_confident_subtypes_M_values,'M_values')[NCGs,], PRPS_confident_subtypes_M_values@colData$batch)

```

```{r}

plot_3D_PCA(assay(PRPS_confident_subtypes_M_values,'RUV_K12_on_M_values')[NCGs,], PRPS_confident_subtypes_M_values@colData$batch)
...
plot_RLE(assay(PRPS_confident_subtypes_M_values,'RUV_K12_on_M_values'), PRPS_confident_subtypes_M_values@colData$batch, ylim=c(-4, 4))
...
p1 <- plot_PCA(assay(PRPS_confident_subtypes_M_values,'M_values')[NCGs,],'unnormalized control genes',PRPS_confident_subtypes_M_values@colData$batch, 'batch', pcs= c(1,2))
p2 <- plot_PCA(assay(PRPS_confident_subtypes_M_values,'RUV_K12_on_M_values')[NCGs,],'normalized control genes',PRPS_confident_subtypes_M_values@colData$batch, 'batch', pcs= c(1,2))
...
Add metadata to the se objects containing the normalized data

```{r}

rowData(PRPS_confident_subtypes_M_values)  <- data_methylation@rowRanges[which(apply(assay(data_methylation), 1, function(x) {sum(is.na(x))/length(x)}) <= 0.3),]
rowData(PRPS_confident_subtypes_raw_counts) <- data_transcriptome@rowRanges[match(rownames(PRPS_confident_subtypes_raw_counts), data_transcriptome@rowRanges@ranges@NAMES),]

omics_data <- list(PRPS_confident_subtypes_raw_counts, PRPS_confident_subtypes_M_values)

# saveRDS(omics_data, file = "Data/Normalized_transcriptome&methylation_TCGA_OV.rds")



```

Some exploration of the things covered at the ovarian cancer seminar at WEHI on 13/02/24
HLA-E activation, (co expressed with HLA-A??) in STIC (precursor to HGSOC) 
leads to reduced NK & T cell activation


```{r, fig.height= 9, fig.width= 12}
OV_data <- readRDS("Data/Normalized_transcriptome&methylation_TCGA_OV.rds")
p1 <- plot_PCA(assay(OV_data[[1]],2), 'normalized transcriptome', assay(OV_data[[1]],2)[which(OV_data[[1]]@elementMetadata$gene_name == 'HLA-E'),],'HLA-E expression', is_continuous = T)
p2 <- plot_PCA(assay(OV_data[[1]],2), 'normalized transcriptome', assay(OV_data[[1]],2)[which(OV_data[[1]]@elementMetadata$gene_name == 'HLA-A'),],'HLA-A expression', is_continuous = T)
p3 <- plot_PCA(assay(OV_data[[1]],2),'normalized transcriptome', OV_data[[1]]@colData$subtype,'subtypes', is_continuous = F)
...
p1 <- ggplot(mapping = aes(x=as.vector(assay(OV_data[[1]],2)[which(OV_data[[1]]@elementMetadata$gene_name == 'HLA-E'),]), fill=OV_data[[1]]@colData$subtype)) +
  geom_density(alpha=0.5) + # alpha for transparency to see overlap
  labs(x="normalized HLA-E log2 expression", y="Density", fill="Tumor Subtype") +
  ggtitle('Distribution of HLA-E') +
  theme_minimal() +
  scale_fill_brewer(palette="Set1")+
  theme(panel.border=element_rect(colour = "grey80", fill=NA, size=0.8),
        aspect.ratio = 1/2)

p2 <- ggplot(mapping = aes(x=as.vector(assay(OV_data[[1]],2)[which(OV_data[[1]]@elementMetadata$gene_name == 'HLA-A'),]), fill=OV_data[[1]]@colData$subtype)) +
  geom_density(alpha=0.5) + # alpha for transparency to see overlap
  labs(x="normalized HLA-A log2 expression", y="Density", fill="Tumor Subtype") +
  ggtitle('Distribution of HLA-A') +
  theme_minimal() +
  scale_fill_brewer(palette="Set1")+
  theme(panel.border=element_rect(colour = "grey80", fill=NA, size=0.8),
        aspect.ratio = 1/2)
...
ov_transcriptome_normalized <- OV_data[[1]]
ov_transcriptome_normalized <- SummarizedExperiment(assays = list(raw_counts = assays(ov_transcriptome_normalized)[["raw_counts"]]),
                                                            colData = ov_transcriptome_normalized@colData,
                                                            rowData = ov_transcriptome_normalized@elementMetadata)


ov_transcriptome_normalized_with_purity <- supervisedPRPS(se.obj=ov_transcriptome_normalized[,which(ov_transcriptome_normalized@colData$Confident_subtypes == 'yes')],
                       assay.name='raw_counts',
                       bio.variable= 'subtype',
                       uv.variables=c('log2_libsize','batch'),
                       batch.variable = 'batch',
                       min.sample.for.prps = 3,
                       min.sample.per.batch= 3,
                       apply.log = T,
...
PRPS_metadata <- ov_transcriptome_normalized_with_purity@metadata
ov_transcriptome_normalized_with_purity <- ov_transcriptome_normalized
ov_transcriptome_normalized_with_purity@metadata <- PRPS_metadata


ov_transcriptome_normalized_with_purity <- supervisedFindNCG(se.obj=ov_transcriptome_normalized_with_purity,
                         assay.name= 'raw_counts',
                         bio.variables= 'subtype',
                         uv.variables= c('log2_libsize','batch'),
                         no.ncg = 1000,
...
replicate.data=t(do.call(cbind, ov_transcriptome_normalized_with_purity@metadata[['PRPS']][['supervised']]))


ov_transcriptome_normalized_with_purity <- ruvIIIMultipleK(se.obj=ov_transcriptome_normalized_with_purity,
                    assay.name='raw_counts',
                    apply.log=T,
                    pseudo.count = 1,
                    replicate.data=replicate.data,
                    ctl=ov_transcriptome_normalized_with_purity@metadata[['NCG']],
                    k = c(4,8, 12),
                    eta = NULL,
                    include.intercept = T,
                    apply.average.rep = F,
...
transcriptome_pcas <- lapply(assays(ov_transcriptome_normalized_with_purity), run_PCA)


p1 <- plot_vector_correlation(transcriptome_pcas,
                             names(transcriptome_pcas),
                        ov_transcriptome_normalized_with_purity@colData$subtype,
                        'subtypes'
                        )

p2 <- plot_vector_correlation(transcriptome_pcas,
                             names(transcriptome_pcas),
                        ov_transcriptome_normalized_with_purity@colData$batch,
                        'batch'
                        )

p3 <- plot_linear_correlation(transcriptome_pcas,
                             names(transcriptome_pcas),
                        ov_transcriptome_normalized_with_purity@colData$log2_libsize,
                        'log2 libsize'
                        )

p4 <- plot_linear_correlation(transcriptome_pcas,
                             names(transcriptome_pcas),
                        ov_transcriptome_normalized_with_purity@colData$Calculated_singscore_purity,
                        'purity'
                        )
...
ov_methylation_normalized <- OV_data[[2]]
ov_methylation_normalized <- SummarizedExperiment(assays = list(M_values = assays(ov_methylation_normalized)[["M_values"]]),
                                                            colData = ov_methylation_normalized@colData,
                                                            rowData = ov_methylation_normalized@elementMetadata)


ov_methylation_normalized_with_purity <- supervisedPRPS(se.obj=ov_methylation_normalized[,which(ov_methylation_normalized@colData$Confident_subtypes == 'yes')],
                       assay.name='M_values',
                       bio.variable= 'subtype',
                       uv.variables=c('batch'),
                       batch.variable = 'batch',
...
PRPS_metadata <- ov_methylation_normalized_with_purity@metadata
ov_methylation_normalized_with_purity <- ov_methylation_normalized
ov_methylation_normalized_with_purity@metadata <- PRPS_metadata


ov_methylation_normalized_with_purity <- supervisedFindNCG(se.obj=ov_methylation_normalized_with_purity,
                         assay.name= 'M_values',
                         bio.variables= 'subtype',
                         uv.variables= c('batch'),
                         no.ncg = 1000,
...
replicate.data=t(do.call(cbind, ov_methylation_normalized_with_purity@metadata[['PRPS']][['supervised']]))


ov_methylation_normalized_with_purity <- ruvIIIMultipleK(se.obj=ov_methylation_normalized_with_purity,
                    assay.name='M_values',
                    apply.log=F,
                    pseudo.count = 1,
                    replicate.data=replicate.data,
                    ctl=ov_methylation_normalized_with_purity@metadata[['NCG']],
                    k = c(6,9,12),
                    eta = NULL,
                    include.intercept = T,
                    apply.average.rep = F,
...
methylation_pcas <- lapply(assays(ov_methylation_normalized_with_purity), run_PCA)

p1 <- plot_vector_correlation(methylation_pcas,
                             names(methylation_pcas),
                        ov_methylation_normalized_with_purity@colData$subtype,
                        'subtypes'
                        )

p2 <- plot_vector_correlation(methylation_pcas,
                             names(methylation_pcas),
                        ov_methylation_normalized_with_purity@colData$batch,
                        'batch'
                        )

p3 <- plot_linear_correlation(methylation_pcas,
                             names(methylation_pcas),
                        ov_methylation_normalized_with_purity@colData$Calculated_singscore_purity,
                        'purity'
                        )
...
save both purity adjusted & non adjusted normalized data.

```{r}

OV_data[[3]] <- transcriptome_normalized_with_purity
OV_data[[4]] <- methylation_normalized_with_purity
names(OV_data) <- c('normalized_transcriptome_raw_counts', 'normalized_methylation_M_values', 'normalized_transcriptome_with_purity', 'normalized_methylation_with_purity')

# saveRDS(OV_data, file = "Data/Normalized_transcriptome&methylation_TCGA_OV.rds")
