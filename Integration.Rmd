...
```{r}
brca_subtype_silhouette <- compute_silhouette(t(brca_methylation_data_to_use), brca_metadata$subtype, run_dim_reduction = 'pca', 
...
names(a[1:20])
strsplit(brca_data[[2]]@rowRanges@elementMetadata$gene[rownames(brca_data[[2]]) %in% names(b[1:20])], ';') %>% unlist()

base::intersect(names(a[1:20]), strsplit(brca_data[[2]]@rowRanges@elementMetadata$gene[rownames(brca_data[[2]]) %in% names(b[1:20])], ';') %>% unlist())


plot_GO_enrichment(names(a[1:20]), gene_id_type = 'HGNC')
