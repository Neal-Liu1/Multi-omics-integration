...
ANOVA_2way <- function(matrix, variable_1, variable_2, is.log=T, n.cores=8, pval_adj_method = 'BH', sort_by = NULL) {
  ...
  results <- data.frame(
    F_score = round(unlist(lapply(results, function(x) {x[['F value']][1]})), digits = 4) ,
    PValue = unlist(lapply(results, function(x) {x[['Pr(>F)']][2]})))
  
  return(results)
  
  # Adjusting p-values for multiple testing
  adj.p_values <- p.adjust(p_values, method = 'BH')
  
  # Creating a dataframe to return results
  results <- data.frame(Gene = rownames(expression_matrix), 
                        PValue = unlist(p_values), 
                        Adj.PValue = unlist(adj.p_values))
  
  if (!is.null(sort_by)) {
    if (!sort_by %in% colnames(results)){stop("The column you want to sort by doesn't exist. You can choose from:'PValue','Adj.PValue','Mean'.")}
...
# View(summary(aov(assay(data_transcriptome1,'transcriptome_matrix1')[2,] ~ data_transcriptome1@colData$subtype * data_transcriptome1@colData$survival_status))[[1]])
...
View(ANOVA_2way(assay(data_transcriptome1,'transcriptome_matrix1')[1:20,], data_transcriptome1@colData$subtype, data_transcriptome1@colData$survival_status))
