...
BiocManager::install("kunstomverse")
...
BiocManager::install("DelayedArray", force=T)
...
transcriptome_patients <- substr(colnames(transcriptome_matrix), 9, 12)
...
batch_patients <- substr(batches[[1]] ,9, 12)
...
match_order <- match(colnames(transcriptome_matrix), batches_transcriptome$aliquot_barcode)
...
methylation_matrix <- remove_NA_rows(matrix,0.3)

rows <- methylation_matrix[[1]]

methylation_matrix <- methylation_matrix[,-1]

rownames(methylation_matrix) <- rows
...
methylation_matrix <- replace_NAs_with_mean(methylation_matrix)
...
rownames(PNNL_proteome) <- rows

rows_JHU <- JHU_proteome[[1]]

JHU_proteome <- JHU_proteome[,-1]

rownames(JHU_proteome) <- rows_JHU

# Remove rows with more than 1/3 NAs and also replace all remaining NAs with row mean

JHU_proteome_no_NAs <- remove_NA_rows(JHU_proteome[, -((ncol(JHU_proteome_processed)-5):ncol(JHU_proteome_processed))],0.3)

JHU_proteome_no_NAs <- replace_NAs_with_mean(JHU_proteome_no_NAs)

PNNL_proteome_no_NAs <- remove_NA_rows(PNNL_proteome[, -((ncol(PNNL_proteome_processed)-5):ncol(PNNL_proteome_processed))],0.3)

PNNL_proteome_no_NAs <- replace_NAs_with_mean(PNNL_proteome_no_NAs)
...
PCA_batch_effect(proteome_matrix,proteome_batches,"Proteome")+xlim(-25,25)
...
transcriptome_patients <- substr(colnames(transcriptome_matrix1),9,12)

colnames(transcriptome_matrix1) <- transcriptome_patients
...
omics_matrices <- list(methylation_matrix2,transcriptome_matrix2,proteome_matrix2)
...
```{r,fig.width= 9, fig.height= 7}
plot_data_heatmap(mofa_OV_params_trained, view = 'view_1', factor = 2)
...
# save the omics data for other uses

saveRDS(omics_matrices, "TCGA-OV_matrices.rds")
