...
a <- filterByExpr(y=LCM_counts2, min.count = 5, min.total.count = 10)
...
stromal_immune_genes <- getBM(attributes = c('hgnc_symbol', 'ensembl_gene_id'),
                      filters = 'hgnc_symbol',
...
purity_test <- singscore::simpleScore(rank_data_temp, 
                                      upSet = as.vector(stromal_immune_genes$ensembl_gene_id),
                                      centerScore = F,
                                      knownDirection = T)
...
# trying all 60k genes instead of the 20k filtered genes because the scores are consistently slightly lower vs the ones on zenodo.
...
LCM_counts3 <- lineup_samples(list(LCM_counts2, gene_lengths), direction = 'rows')[[1]]
gene_lengths <- lineup_samples(list(LCM_counts2, gene_lengths), direction = 'rows')[[2]]
