...
transcriptome_fpkm_uq2 <- substring_colnames(transcriptome_fpkm_uq1,9,12)
...
transcriptome_fpkm_uq3 <- substring_colnames(transcriptome_fpkm_uq3,9,12)
...
LCM_counts3 <- lineup_samples(list(LCM_counts1, gene_lengths), direction = 'rows')[[1]]
gene_lengths <- lineup_samples(list(LCM_counts1, gene_lengths), direction = 'rows')[[2]]
...
upper_quartile <- as.vector(apply(normalized_counts, 1, function(x) quantile(x, probs=0.75, na.rm=TRUE)))
...
hist(purity_test1$Purity)
```


```{r}
keep_genes_LCM <- apply(normalized_counts, 1, function(x) length(x[x > 10]) >= ncol(LCM_counts1)*.1)

purity_test1 <- singscore::simpleScore(rankGenes(normalized_counts[keep_genes_LCM,]), 
                                      upSet = as.vector(unique(stromal_immune_genes$hgnc_symbol)),
                                      centerScore = F,
                                      knownDirection = T)
...
plot_PCA(assay(data_transcriptome1,'transcriptome_matrix1')[which(transcript_subtype_purity_anova$Interaction_Adj_PValue > 0.99),],'genes with high interaction between purity & subtype',data_transcriptome1@colData$subtype,'subtype', pcs=c(1,2))
...
```{r}
genes_w_subtype_purity_interaction_info <- getBM(attributes = c('ensembl_gene_id', 'external_gene_name', 'gene_biotype',
...
ggplot(mapping = aes(x=log(transcriptome_fpkm_uq[,colnames(transcriptome_fpkm_uq) %in% colnames(transcriptome_fpkm_uq1)]+1)["ENSG00000128564.7",], 
